...
export const SearchElement = (game: Record<string, string>, key: number) => {
  ...
  const handleClick = () => {
    navigate(`/game/${encodeURIComponent(game.title)}`);
  };
  // console.log(game);
  return (
    <button
      className="flex flex-row gap-2 bg-gray-300 p-4 rounded-md h-auto items-center border-2 hover:border-gray-400 transition ease-linear"
      key={key}
      onClick={handleClick}
    >
      <img
      ...
      <div className="flex flex-col ml-3 self-start">
        ...
        <h3 className="text-sm font-light self-start">
          {releaseDate.getMonth() + 1}-{releaseDate.getDate()}-
